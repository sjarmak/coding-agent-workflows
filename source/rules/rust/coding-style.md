...
- **Scaffold**: `cargo new <crate>` (binary) or `cargo new <crate> --lib` (library). Use a workspace (`cargo new --workspace`) when you have more than one crate in the repo.
- **Standard directories**: `src/` (see Module Organization above), `tests/` for integration tests, `benches/` for criterion benchmarks, `examples/` for runnable samples. Workspaces use `crates/<name>/` or a flat layout.
- **Toolchain completeness**: `rustfmt`, `clippy -- -D warnings`, and **`cargo-deny`** for dependency/license auditing. Pin `rust-toolchain.toml` to a specific channel for reproducibility.
