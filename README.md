...
scaffolds a **thin, per-project** `AGENTS.md` that holds only this repo's
intention and failure-mode preventions and *points* to the bundle for everything
else. See [Project context layers](#project-context-layers) below.
...
project's** `AGENTS.md` should be the opposite — thin. `install.sh init` plus the
...
- **`AGENTS.md`** (per project, thin) — this repo's *intention* and
  *failure-mode preventions*, plus *pointers* to the rest. Target ~120 lines.
- **Compass files** (`COMPASS.md` per area) — the tribal knowledge: the *why*,
  the gotchas, how an area connects. Generated by `project-compass`, kept fresh
  with a content-hash, indexed (not duplicated) by `AGENTS.md`.
