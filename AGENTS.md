...
A curated, agent-neutral set of coding practices, agent roles, skills, and workflows. 
Claude Code reads `targets/claude/`; Codex reads this file plus `targets/codex/`; 
any agent that reads `AGENTS.md` (Amp, Cody, Aider, Gemini CLI, …) gets the universal layer below.
...
- **simplify** — Reduce a diff to its essential complexity — remove dead paths, over-engineering, premature abstraction, and incidental churn — without changing behavior.
- **code-review** — 
- **distill** — 
